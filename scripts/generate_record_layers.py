...
def main():
    ...
    print("\n[1/2] Parsing msd-transfer headers...")
    records = parse_record_headers(headers_dir)
    print(f"  Found {len(records)} records")

    # Phase 2: Generate pybind bindings
    print("\n[2/2] Generating pybind11 bindings...")
    codegen = PybindCodegen()
    pybind_code = codegen.generate(records)

    if args.check_only:
        # Check mode: compare against committed file
        if not pybind_output.exists():
            print(f"  ERROR: {pybind_output} does not exist", file=sys.stderr)
            sys.exit(1)

        existing_code = pybind_output.read_text()
        if existing_code != pybind_code:
            print(f"  ERROR: Generated code differs from {pybind_output}", file=sys.stderr)
            print("  Run: python scripts/generate_record_layers.py", file=sys.stderr)
            sys.exit(1)

        print(f"  OK: Generated code matches {pybind_output}")
    else:
