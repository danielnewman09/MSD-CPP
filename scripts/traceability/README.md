...
Parses C++ transfer records (`BOOST_DESCRIBE_STRUCT`), pybind11 bindings, and Pydantic models to extract field lists and build cross-layer mappings. Provides visibility into which fields are connected across layers and which have drifted.

**Introduced**: [Ticket 0061](../../tickets/0061_cross_layer_record_mapping.md)

**Four layers indexed**:
1. **C++ transfer records** (`msd-transfer/*.hpp`) — `BOOST_DESCRIBE_STRUCT` field lists
2. **SQL schema** — auto-generated by `cpp_sqlite` (types inferred from C++ fields)
3. **pybind11 bindings** (`msd-pybind/src/record_bindings.cpp`) — Python-exposed fields
4. **Pydantic models** (`replay/replay/models.py`) — REST API response schemas

- **Tables populated**: `record_layer_fields`, `record_layer_mapping`, `record_layer_fields_fts`
- **Incremental**: Clears and rebuilds on each run (fast, ~1 second)
- **Typical output**: ~28 C++ records, ~26 pybind classes, ~14 Pydantic models
