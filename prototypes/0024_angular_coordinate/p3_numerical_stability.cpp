...
// Purpose: Test long-duration simulation to compare angle drift with different normalization frequencies
...
public:
  ...
  double pitch() const { return normalizeAngle((*this)[0]); }
  double roll() const { return normalizeAngle((*this)[1]); }
  double yaw() const { return normalizeAngle((*this)[2]); }
};

// Strategy 2: Normalize periodically (every N steps)
class AngularCoordinate_Periodic : public Eigen::Vector3d {
public:
  AngularCoordinate_Periodic() : Eigen::Vector3d{0.0, 0.0, 0.0}, normalizePeriod_{1000}, stepCount_{0} {}

  AngularCoordinate_Periodic(double pitch, double roll, double yaw, size_t period = 1000)
      : Eigen::Vector3d{pitch, roll, yaw}, normalizePeriod_{period}, stepCount_{0} {}

  template <typename OtherDerived>
  AngularCoordinate_Periodic(const Eigen::MatrixBase<OtherDerived>& other)
      : Eigen::Vector3d{other}, normalizePeriod_{1000}, stepCount_{0} {}

  template <typename OtherDerived>
  AngularCoordinate_Periodic& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    this->Eigen::Vector3d::operator=(other);
    stepCount_++;
  ...
  double pitch() const { return (*this)[0]; }  // No normalization on access
...
template <typename AngularType>
SimulationResult<AngularType> runSimulation(const std::string& name, size_t timeSteps) {
  ...
  for (size_t i = 0; i < timeSteps; ++i) {
    ...
    if (std::abs(pitch) > 1000.0 || std::abs(roll) > 1000.0 || std::abs(yaw) > 1000.0) {
...
int main() {
  ...
  std::cout << std::format("  Total simulation time: {:.1f}s ({:.1f} hours)\n",
                          simulationTime, simulationTime / 3600.0);
  ...
  auto lazyResult = runSimulation<AngularCoordinate_Lazy>("Lazy (normalize on every access)", timeSteps);
  auto periodic1000Result = runSimulation<AngularCoordinate_Periodic>("Periodic (every 1000 steps)", timeSteps);
  auto periodic10000Result = runSimulation<AngularCoordinate_Periodic>("Periodic (every 10000 steps)", timeSteps);
  auto neverResult = runSimulation<AngularCoordinate_Never>("Never normalize", timeSteps);
  ...
  auto printRow = [&](const auto& r) {
    std::cout << std::format("{:<35} {:>11.6f} {:>11.6f} {:>11.6f}\n",
                            r.name, r.finalPitch, r.finalRoll, r.finalYaw);
  };
  ...
  std::cout << std::format("{:<35} {:>12} {:>12} {:>12}\n", "Strategy", "Max Pitch", "Max Roll", "Max Yaw");
  std::cout << std::format("{:-<71}\n", "");

  auto printMaxRow = [&](const auto& r) {
    std::cout << std::format("{:<35} {:>11.6f} {:>11.6f} {:>11.6f}\n",
                            r.name, r.maxPitch, r.maxRoll, r.maxYaw);
  };
  ...
  bool lazyStable = (lazyResult.maxPitch < 2.0 * M_PI && lazyResult.maxRoll < 2.0 * M_PI &&
                     lazyResult.maxYaw < 2.0 * M_PI);
  bool periodic1000Stable = (periodic1000Result.maxPitch < 100.0 && periodic1000Result.maxRoll < 100.0 &&
                             periodic1000Result.maxYaw < 100.0);
  bool periodic10000Stable = (periodic10000Result.maxPitch < 1000.0 && periodic10000Result.maxRoll < 1000.0 &&
                              periodic10000Result.maxYaw < 1000.0);

  if (lazyStable) {
    std::cout << "✅ LAZY normalization: Angles remain bounded within (-π, π]\n";
    std::cout << "   Maximum angle magnitude: "
              << std::format("{:.6f} rad\n", std::max({lazyResult.maxPitch, lazyResult.maxRoll, lazyResult.maxYaw}));
  } else {
    std::cout << "❌ LAZY normalization: Angles exceeded expected bounds!\n";
  }

  if (periodic1000Stable) {
    std::cout << "✅ PERIODIC (1000 steps) normalization: Angles remain bounded\n";
    std::cout << "   Maximum angle magnitude: "
              << std::format("{:.6f} rad\n", std::max({periodic1000Result.maxPitch, periodic1000Result.maxRoll, periodic1000Result.maxYaw}));
  } else {
    std::cout << "⚠️  PERIODIC (1000 steps) normalization: Angles growing, may overflow\n";
  }

  if (periodic10000Stable) {
    std::cout << "✅ PERIODIC (10000 steps) normalization: Angles remain bounded\n";
    std::cout << "   Maximum angle magnitude: "
              << std::format("{:.6f} rad\n", std::max({periodic10000Result.maxPitch, periodic10000Result.maxRoll, periodic10000Result.maxYaw}));
  } else {
    std::cout << "⚠️  PERIODIC (10000 steps) normalization: Angles growing, may overflow\n";
  }

  std::cout << "\n❌ NEVER normalize: Angles grow unbounded ("
            << std::format("{:.1f} rad max)\n", std::max({neverResult.maxPitch, neverResult.maxRoll, neverResult.maxYaw}));
  std::cout << "   This is expected - angles accumulate over "
            << std::format("{:.1f} revolutions\n", std::max({0.1 * simulationTime, 0.05 * simulationTime, 0.15 * simulationTime}) / (2.0 * M_PI));
  ...
  if (lazyStable) {
    ...
    std::cout << "  ⚠️  LAZY normalization may have issues - investigate further\n";
  }
