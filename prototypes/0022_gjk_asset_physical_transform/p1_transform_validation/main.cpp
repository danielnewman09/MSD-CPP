...
// Question: Does the transformation pipeline (globalToLocalRelative → vertex
// search → localToGlobal)
//           produce correct world-space support vertices for known geometries
//           and transforms?
// Success criteria:
//   - Support vertices match analytical predictions for unit cube with
//   translation-only transform
//   - Support vertices match analytical predictions for unit cube with
//   rotation-only transform (90° about z-axis)
//   - Support vertices match analytical predictions for unit cube with combined
//   translation + rotation
//   - Identity transform produces identical results to raw ConvexHull support
//   function
...
public:
  SimpleReferenceFrame()
    : origin_{0, 0, 0}, rotation_{Eigen::Matrix3d::Identity()}
  {
  }

  explicit SimpleReferenceFrame(const Coordinate& origin)
    : origin_{origin}, rotation_{Eigen::Matrix3d::Identity()}
  {
  }
...
Coordinate supportFunction(const std::vector<Coordinate>& vertices,
...
bool approxEqual(const Coordinate& a,
                 const Coordinate& b,
                 double epsilon = 1e-6)
{
...
int main()
{
  std::cout << "=== Prototype P1: Transformation Correctness Validation ==="
            << std::endl;
  std::cout << std::endl;
  ...
  // Test 1: Identity transform should match raw support function
  {
    ...
    for (const auto& dir : testDirections)
    {
      ...
      Coordinate transformedSup =
        transformedSupport(cubeVertices, identityFrame, dir);

      if (!approxEqual(rawSupport, transformedSup))
      {
        std::cout << "  FAIL: Direction " << std::format("{:.2f}", dir)
                  << std::endl;
        std::cout << "    Raw:        " << std::format("{:.6f}", rawSupport)
                  << std::endl;
        std::cout << "    Transformed: "
                  << std::format("{:.6f}", transformedSup) << std::endl;
        test1Pass = false;
      }
    }
  ...
  {
    std::cout << "Test 2: Translation-only transform (translate by (10, 0, 0))"
              << std::endl;
    std::cout << "Expected: Support vertices shifted by translation"
              << std::endl;
    ...
    // - Direction (1,0,0): max vertex is (0.5, y, z) → becomes (10.5, y, z).
    // Picks arbitrary y,z with max dot.
    //   Since dot = 0.5*1 + y*0 + z*0 = 0.5 for all y,z, it picks the first
    //   one: (0.5, -0.5, -0.5) → (10.5, -0.5, -0.5)
    std::vector<TestCase> testCases = {{Coordinate{1, 0, 0},
                                        Coordinate{10.5, -0.5, -0.5},
                                        "+X direction -> (10.5, -0.5, -0.5)"},
                                       {Coordinate{-1, 0, 0},
                                        Coordinate{9.5, -0.5, -0.5},
                                        "-X direction -> (9.5, -0.5, -0.5)"},
                                       {Coordinate{0, 1, 0},
                                        Coordinate{9.5, 0.5, -0.5},
                                        "+Y direction -> (9.5, 0.5, -0.5)"},
                                       {Coordinate{0, -1, 0},
                                        Coordinate{9.5, -0.5, -0.5},
                                        "-Y direction -> (9.5, -0.5, -0.5)"},
                                       {Coordinate{0, 0, 1},
                                        Coordinate{9.5, -0.5, 0.5},
                                        "+Z direction -> (9.5, -0.5, 0.5)"},
                                       {Coordinate{0, 0, -1},
                                        Coordinate{9.5, -0.5, -0.5},
                                        "-Z direction -> (9.5, -0.5, -0.5)"}};
    ...
    {
      Coordinate result =
        transformedSupport(cubeVertices, translatedFrame, tc.direction);

      if (!approxEqual(result, tc.expectedSupport))
      {
        std::cout << "  FAIL: " << tc.description << std::endl;
        std::cout << "    Expected: "
                  << std::format("{:.6f}", tc.expectedSupport) << std::endl;
        std::cout << "    Got:      " << std::format("{:.6f}", result)
                  << std::endl;
        test2Pass = false;
      }
    }
  ...
  {
    std::cout << "Test 3: Rotation-only transform (90° CCW about Z-axis)"
              << std::endl;
    ...
    // - Local support for (0,1,0): vertex (x, 0.5, z) where x,z arbitrary →
    // (0.5, 0.5, -0.5)
    ...
    std::vector<TestCase> testCases = {{Coordinate{1, 0, 0},
                                        Coordinate{0.5, 0.5, -0.5},
                                        "+X direction (max in rotated frame)"},
                                       {Coordinate{0, 1, 0},
                                        Coordinate{-0.5, 0.5, -0.5},
                                        "+Y direction (max in rotated frame)"},
                                       {Coordinate{0, 0, 1},
                                        Coordinate{0.5, -0.5, 0.5},
                                        "+Z direction (max vertex for +Z)"}};

    bool test3Pass = true;
    for (const auto& tc : testCases)
    {
      Coordinate result =
        transformedSupport(cubeVertices, rotatedFrame, tc.direction);
  ...
  {
    std::cout
      << "Test 4: Combined transform (translate (5, 0, 0) + 90° CCW about Z)"
      << std::endl;
    std::cout << "Expected: Support vertices rotated then translated"
              << std::endl;
    ...
    std::vector<TestCase> testCases = {
      {Coordinate{1, 0, 0},
       Coordinate{5.5, 0.5, -0.5},
       "+X direction (rotated + translated)"},
      {Coordinate{0, 1, 0},
       Coordinate{4.5, 0.5, -0.5},
       "+Y direction (rotated + translated)"},
    ...
    {
      Coordinate result =
        transformedSupport(cubeVertices, combinedFrame, tc.direction);
