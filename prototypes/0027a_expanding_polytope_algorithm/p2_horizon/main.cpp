...
// Success criteria: No duplicate edges, all faces valid, polytope remains
// closed
    ...
    for (const auto& [edge, count] : edgeCounts)
    {
      if (count != 2)
      {
        ...
        metrics.failureReason =
          std::format("Edge ({},{}) shared by {} faces (expected 2)",
                      edge.v0,
                      edge.v1,
                      count);
        return metrics;
...
public:
  ...
  TopologyMetrics testHorizonEdges(
    const std::vector<Coordinate>& initialSimplex,
    const std::vector<Coordinate>& expansionPoints)
  {
    ...
    for (size_t i = 0; i < expansionPoints.size(); ++i)
    {
      ...
      TopologyMetrics preMetrics =
        TopologyValidator::validatePolytope(vertices_, faces_);
      if (!preMetrics.isValid)
      {
        overallMetrics = preMetrics;
        overallMetrics.failureReason =
          std::format("Topology invalid BEFORE expansion {}: {}",
                      i,
                      preMetrics.failureReason);
      ...
      if (uniqueHorizon.size() != horizon.size())
      {
        ...
        overallMetrics.duplicateEdgeCount =
          horizon.size() - uniqueHorizon.size();
        overallMetrics.failureReason =
      ...
      TopologyMetrics postMetrics =
        TopologyValidator::validatePolytope(vertices_, faces_);
      if (!postMetrics.isValid)
      {
        overallMetrics = postMetrics;
        overallMetrics.failureReason =
          std::format("Topology invalid AFTER expansion {}: {}",
                      i,
                      postMetrics.failureReason);
        return overallMetrics;
      }
...
private:
  ...
  std::vector<EPAEdge> buildHorizonEdges(const Coordinate& newVertex)
  {
    ...
    {
      int count =
        std::count(edgeCandidates.begin(), edgeCandidates.end(), edge);
      if (count == 1)
      {
...
std::vector<TestCase> generateTestCases()
{
  ...
  cases.push_back({"Simple expansion (4 points)",
                   {Coordinate{0.5, 0, 0},
                    Coordinate{0, 0.5, 0},
                    Coordinate{0, 0, 0.5},
  ...
  // Case 2: Many iterations
  {
    ...
    cases.push_back({"Many expansions (20 points)",
                     {Coordinate{0.4, 0, 0},
                      Coordinate{0, 0.4, 0},
                      Coordinate{0, 0, 0.4},
                      Coordinate{-0.1, -0.1, -0.1}},
  ...
  cases.push_back({"Near-coplanar expansion",
...
int main()
{
  std::cout << "Prototype P2: Horizon Edge Construction Robustness"
            << std::endl;
  std::cout << "Question: Does horizon edge detection maintain valid topology?"
            << std::endl;
  std::cout
    << "Success criteria: No duplicates, valid normals, watertight polytope"
    << std::endl;
  ...
  for (const auto& testCase : testCases)
  {
    ...
    if (!metrics.isValid)
    {
      std::cout << std::format("    Failure: {}", metrics.failureReason)
                << std::endl;
    }
  ...
  std::cout << std::format("Failed:              {}", totalTests - passedTests)
            << std::endl;
  ...
  std::cout << std::format("✓ No duplicate horizon edges: {} (all unique)",
            ...
            << std::endl;
  std::cout << std::format(
                 "✓ All faces valid normals:    {} (non-zero, finite)",
                 overallPass ? "PASS" : "CHECK FAILURES")
            << std::endl;
  std::cout << std::format(
                 "✓ Polytope watertight:        {} (edges shared by 2 faces)",
                 overallPass ? "PASS" : "CHECK FAILURES")
            << std::endl;
  std::cout << std::format(
                 "✓ No topology corruption:     {} (100+ expansions tested)",
                 overallPass ? "PASS" : "CHECK FAILURES")
            << std::endl;
  std::cout << std::string(80, '=') << std::endl;
  ...
  if (!overallPass)
  {
    ...
    std::cout << "  - Consider std::set with custom comparator for edges"
              << std::endl;
    std::cout
