...
// Question: Does the constraint-based approach conserve energy better than impulse-based?
// Success criteria: See README.md
...
public:
    ...
    SimulationResult runImpulseBased() {
        ...
        // Initial conditions
        Eigen::Vector3d position{0.0, 0.0, 5.0};
        ...
        for (int frame = 0; frame < 1000; ++frame) {
            ...
            if (penetrating) {
                ...
                if (v_rel_n < -velocity_threshold_) {
                    // Compute impulse
                    double effective_mass = 1.0 / mass_;  // Ground infinite mass, center contact
        ...
        for (size_t i = 1; i < result.frames.size(); ++i) {
            ...
            if (result.frames[i].total_energy > result.frames[i-1].total_energy + 0.1) {
...
void printResults(const SimulationResult& result) {
    ...
    std::cout << "\nSummary:" << std::endl;
    std::cout << "  Max penetration: " << result.max_penetration << " m" << std::endl;
    std::cout << "  Final Z: " << result.final_z << " m" << std::endl;
    std::cout << "  Final Vz: " << result.final_vz << " m/s" << std::endl;
    std::cout << "  Initial energy: " << result.frames.front().total_energy << " J" << std::endl;
    std::cout << "  Final energy: " << result.frames.back().total_energy << " J" << std::endl;
    std::cout << "  Energy dissipated: "
              << (result.frames.front().total_energy - result.frames.back().total_energy) << " J" << std::endl;
    std::cout << "  Energy monotonic: " << (result.energy_monotonic ? "YES" : "NO") << std::endl;
}

bool checkCriteria(const SimulationResult& impulse, const SimulationResult& constraint) {
    std::cout << "\n=== SUCCESS CRITERIA ===" << std::endl;

    bool all_pass = true;

    // Criterion 1: Both approaches produce decreasing total energy
    std::cout << "\n1. Both approaches produce decreasing total energy over time:" << std::endl;
    double impulse_energy_change = impulse.frames.back().total_energy - impulse.frames.front().total_energy;
    double constraint_energy_change = constraint.frames.back().total_energy - constraint.frames.front().total_energy;
    std::cout << "   Impulse-based energy change: " << impulse_energy_change << " J" << std::endl;
    std::cout << "   Constraint-based energy change: " << constraint_energy_change << " J" << std::endl;
    bool criterion1 = (impulse_energy_change < 0.0) && (constraint_energy_change < 0.0);
    std::cout << "   Result: " << (criterion1 ? "PASS" : "FAIL") << std::endl;
    all_pass &= criterion1;

    // Criterion 2: Constraint approach does not inject energy
    std::cout << "\n2. Constraint approach does NOT inject energy (monotonic):" << std::endl;
    std::cout << "   Energy monotonic: " << (constraint.energy_monotonic ? "YES" : "NO") << std::endl;
    ...
    std::cout << "\n3. Maximum penetration depth for constraint approach < 0.02m:" << std::endl;
    std::cout << "   Max penetration: " << constraint.max_penetration << " m" << std::endl;
    ...
    std::cout << "\n4. Both approaches eventually reach rest (|v| < 0.01 m/s) within 1000 frames:" << std::endl;
    std::cout << "   Impulse-based final |v|: " << std::abs(impulse.final_vz) << " m/s" << std::endl;
    std::cout << "   Constraint-based final |v|: " << std::abs(constraint.final_vz) << " m/s" << std::endl;
    bool criterion4 = (std::abs(impulse.final_vz) < 0.01) && (std::abs(constraint.final_vz) < 0.01);
    ...
    std::cout << "   Impulse-based final Z: " << impulse.final_z << " m" << std::endl;
    std::cout << "   Constraint-based final Z: " << constraint.final_z << " m" << std::endl;
    ...
    std::cout << "\n=== OVERALL: " << (all_pass ? "ALL PASS" : "SOME FAILURES") << " ===" << std::endl;
...
int main() {
    ...
    std::cout << "Comparing impulse-based vs constraint-based approaches" << std::endl;
    std::cout << "\nScenario:" << std::endl;
