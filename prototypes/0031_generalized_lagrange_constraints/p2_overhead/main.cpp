...
public:
    ...
    virtual Eigen::VectorXd evaluate(const MockInertialState& state, double time) const = 0;
    virtual Eigen::MatrixXd jacobian(const MockInertialState& state, double time) const = 0;
...
public:
    ...
    Eigen::VectorXd evaluate(const MockInertialState& state, double /*time*/) const override {
        Eigen::VectorXd C(1);
        C(0) = state.orientation.squaredNorm() - 1.0;
        return C;
    }

    Eigen::MatrixXd jacobian(const MockInertialState& state, double /*time*/) const override {
...
int main() {
    ...
    std::cout << "Testing performance of constraint framework vs. hard-coded implementation\n\n";
    ...
    double baselineEvalNs = benchmark("Baseline: positionViolation", [&]() {
    ...
    double virtualEvalNs = benchmark("Virtual: evaluate()", [&]() {
    ...
    double evalOverhead = ((virtualEvalNs - baselineEvalNs) / baselineEvalNs) * 100.0;
    std::cout << "  Overhead: " << std::fixed << std::setprecision(2) << evalOverhead << "%\n\n";
    ...
    double baselineEvalJacNs = benchmark("Baseline: eval + implicit jacobian", [&]() {
        QuaternionConstraintBaseline baseline;
        double g = baseline.positionViolation(state.orientation);
        auto force = baseline.computeConstraintForce(state.orientation, state.quaternionRate);
        jac_sink = g + force.norm();
    }, kIterations);
    printBenchmarkResult("Baseline: eval + force", baselineEvalJacNs);

    double virtualEvalJacNs = benchmark("Virtual: evaluate() + jacobian()", [&]() {
        UnitQuaternionConstraint constraint;
        Constraint* c = &constraint;
    ...
    double evalJacOverhead = ((virtualEvalJacNs - baselineEvalJacNs) / baselineEvalJacNs) * 100.0;
    std::cout << "  Overhead: " << std::fixed << std::setprecision(2) << evalJacOverhead << "%\n\n";
    ...
    // For a fair comparison, measure just the evaluation + constraint force computation
    // (not the full matrix solve, which the baseline doesn't do)
    double baselineComparableNs = benchmark("Baseline: eval + constraint force", [&]() {
        QuaternionConstraintBaseline baseline;
        double g = baseline.positionViolation(state.orientation);
        double g_dot = baseline.velocityViolation(state.orientation, state.quaternionRate);
        auto force = baseline.computeConstraintForce(state.orientation, state.quaternionRate);
        global_sink = g + g_dot + force.norm();
    }, kIterations);
    printBenchmarkResult("Baseline: eval + constraint force", baselineComparableNs);

    double virtualComparableNs = benchmark("Virtual: eval + jacobian + alpha/beta", [&]() {
        UnitQuaternionConstraint constraint;
        Constraint* c = &constraint;
        auto C = c->evaluate(state, 0.0);
        auto J = c->jacobian(state, 0.0);
    ...
    printBenchmarkResult("Virtual: eval + jacobian + alpha/beta", virtualComparableNs);

    double comparableOverhead = ((virtualComparableNs - baselineComparableNs) / baselineComparableNs) * 100.0;
    std::cout << "  Overhead: " << std::fixed << std::setprecision(2) << comparableOverhead << "%\n\n";
    ...
    double virtualFullNs = benchmark("Virtual: ConstraintSolver::solve()", [&]() {
        UnitQuaternionConstraint constraint;
        ConstraintSolver solver;
        std::vector<Constraint*> constraints{&constraint};
        MockInertialState testState = state;
        auto result = solver.solve(constraints, testState, mass, inverseInertia, dt);
    ...
    std::cout << "  Absolute time per solve: " << std::fixed << std::setprecision(3) << absoluteUs << " us\n\n";
    ...
    std::cout << "Evaluation overhead:            " << std::setw(8) << evalOverhead << "%\n";
    std::cout << "Eval+Jacobian overhead:         " << std::setw(8) << evalJacOverhead << "%\n";
    std::cout << "Comparable operations overhead: " << std::setw(8) << comparableOverhead << "%\n";

    std::cout << "\nAbsolute time per solve:        " << std::setprecision(3) << absoluteUs << " us\n";
    ...
    bool negligibleVirtualOverhead = evalOverhead < 10.0;  // Virtual eval is same cost as baseline

    std::cout << "\nSuccess Criteria:\n";
    std::cout << "  Absolute solve time < 5 us:    " << (absolutePass ? "PASS" : "FAIL");
    std::cout << " (" << std::setprecision(3) << absoluteUs << " us)\n";
    std::cout << "  Virtual eval overhead < 10%:   " << (negligibleVirtualOverhead ? "PASS" : "FAIL");
    std::cout << " (" << std::setprecision(2) << evalOverhead << "%)\n";

    std::cout << "\nInterpretation:\n";
    std::cout << "  The virtual constraint framework adds ~" << std::setprecision(1) << absoluteUs;
    std::cout << " us per solve.\n";
    std::cout << "  Most overhead is from matrix operations (Jacobian, LLT solve),\n";
    std::cout << "  not virtual function calls (which show <1% overhead above).\n";
