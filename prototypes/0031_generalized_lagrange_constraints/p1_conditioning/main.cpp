...
// Question: Do typical constraint combinations produce well-conditioned
// matrices? Success criteria: Condition number < 1e12, graceful handling of
// singular matrices
  ...
  const Eigen::Matrix3d& inverseInertia)
{
  ...
  // Note: For quaternion DOF, we use a simplified 4x4 block with the same
  // inertia pattern
  Eigen::MatrixXd Minv = Eigen::MatrixXd::Zero(7, 7);
  Minv.block<3, 3>(0, 0) = (1.0 / mass) * Eigen::Matrix3d::Identity();

  // Quaternion inertia (simplified): use same inertia as rotation but extend to
  // 4x4
...
int main()
{
  ...
  std::cout
    << "Testing constraint matrix condition numbers and LLT robustness\n";
  ...
  // Test 1: Single quaternion constraint
  {
    ...
    state.orientation =
      Eigen::Quaterniond(0.7071, 0.7071, 0.0, 0.0);  // 90° rotation
    state.position = msd_sim::Vector3D(1.0, 2.0, 3.0);
    ...
    Eigen::MatrixXd A =
      assembleConstraintMatrix(jacobians, mass, inverseInertia);
    ...
    printResult(
      "LLT converged", result.converged ? "true" : "false", convergePass);
    printResult(
      "No NaN in solution", !result.hasNaN ? "true" : "false", nanPass);
  ...
  // Test 2: Quaternion + Distance constraint
  {
    ...
    state.position = msd_sim::Vector3D(5.0, 0.0, 0.0);  // 5m from origin
  ...
  // Test 3: Quaternion + Distance (realistic scenario)
  {
    ...
    state.position = msd_sim::Vector3D(3.0, 4.0, 5.0);
    ...
    DistanceConstraintMock dConstraint{
      std::sqrt(50.0)};  // Distance matches position magnitude
    jacobians.push_back(dConstraint.jacobian(state));
    ...
    std::cout << "State dimension: 7 (3 pos + 4 quat)\n";
    std::cout
      << "Note: Quaternion acts on DOFs 4-7, Distance acts on DOFs 1-3\n";
  ...
  // Test 4: Deliberately singular matrix (zero inertia)
  {
    ...
    state.position = msd_sim::Vector3D(1.0, 0.0, 0.0);
    ...
    // Create singular case: zero inertia (but nonzero mass to avoid division by
    // zero)
    double mass = 10.0;
    ...
    bool failedAsExpected =
      !result.converged || std::isinf(result.conditionNumber);
    // NaN is acceptable for singular matrices as long as we detect failure
    bool failureDetected = failedAsExpected;

    printResult("Condition number",
                result.conditionNumber,
                true);  // Any result is acceptable
    printResult("Failed as expected",
                failedAsExpected ? "true" : "false",
                failedAsExpected);
    printResult(
      "Failure detected", failureDetected ? "true" : "false", failureDetected);
  ...
  // Test 5: Near-singular matrix (very small mass)
  {
    ...
    bool condAcceptable =
      result.conditionNumber < kMaxConditionNumber || result.converged;
    bool noNaN = !result.hasNaN;

    printResult("Condition number", result.conditionNumber, condAcceptable);
    printResult(
      "LLT converged", result.converged ? "true" : "false", result.converged);
  ...
  if (allTestsPassed)
  {
    ...
    std::cout << "Constraint matrices are well-conditioned for typical "
                 "configurations.\n";
