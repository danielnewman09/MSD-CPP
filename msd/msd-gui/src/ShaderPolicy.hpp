...
/**
 * @brief Instance data for Position3DColorTransform shader (position-only rendering)
 * @ticket 0002_remove_rotation_from_gpu
 *
...
/**
 * @brief Instance data for PositionRotation3DColorTransform shader (full transform rendering)
 * @ticket 0002_remove_rotation_from_gpu
 *
 ...
 * Uses Position3DColorTransform.vert shader which applies only translation to geometry.
 *
...
public:
   ...
   *
   * Configures:
   * - Location 0-2: Per-vertex attributes (position, color, normal) from buffer slot 0
   * - Location 3-4: Per-instance attributes (position, color) from buffer slot 1
   */
  std::vector<SDL_GPUVertexAttribute> getVertexAttributes() const;

  /**
   * @brief Get vertex buffer descriptions for both per-vertex and per-instance data
   * @return Vector of buffer descriptions (2 elements: vertex buffer, instance buffer)
   */
 ...
 * Uses PositionRotation3DColorTransform.vert shader which supports translation and rotation.
 *
 * Thread safety: All methods are const and stateless (thread-safe)
 */
...
public:
  ...
  /**
   * @brief Build instance data from simulation object
   * @param object Source object containing transform, color, and geometry reference
   * @param geometryNameToIndex Mapping from geometry names to registry indices
   * @return Serialized instance data ready for GPU upload
   *
   * Extracts full transform (4x4 matrix), color, and geometry index from object.
   */
...
private:
   ...
   */
  Eigen::Matrix4f createModelMatrix(const msd_sim::ReferenceFrame& transform) const;
};
