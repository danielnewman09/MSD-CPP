...
TEST(CollisionResultTest, ParameterizedConstruction_StoresValues)
{
  ...
  EXPECT_DOUBLE_EQ(result.contactPointA.x(), 0.25);
  EXPECT_DOUBLE_EQ(result.contactPointA.y(), 0.0);
  EXPECT_DOUBLE_EQ(result.contactPointA.z(), 0.0);
  EXPECT_DOUBLE_EQ(result.contactPointB.x(), -0.25);
  EXPECT_DOUBLE_EQ(result.contactPointB.y(), 0.0);
  EXPECT_DOUBLE_EQ(result.contactPointB.z(), 0.0);
}
...
TEST(EPATest, WitnessPoints_FaceContact)
{
  ...
  EXPECT_GT(result->contactPointA.x(), 0.0);  // On positive side of A
  EXPECT_LT(result->contactPointA.x(), 0.6);  // Near A's +X face
  EXPECT_GT(result->contactPointB.x(), 0.0);  // On positive side of B (in world space)
  EXPECT_LT(result->contactPointB.x(), 0.5);  // Near B's -X face

  // Y and Z should be near center of contact region
  EXPECT_NEAR(std::abs(result->contactPointA.y()), 0.0, 0.5);
  EXPECT_NEAR(std::abs(result->contactPointA.z()), 0.0, 0.5);
  EXPECT_NEAR(std::abs(result->contactPointB.y()), 0.0, 0.5);
  EXPECT_NEAR(std::abs(result->contactPointB.z()), 0.0, 0.5);
}
...
TEST(EPATest, WitnessPoints_EnableTorqueCalculation)
{
  ...
  Coordinate leverArmA = result->contactPointA - centerOfMassA;
  Coordinate leverArmB = result->contactPointB - centerOfMassB;
...
TEST(EPATest, WitnessPoints_DifferentForDifferentCollisions)
{
  ...
  double xDiff = std::abs(result1->contactPointA.x() - result2->contactPointA.x());
  double yDiff = std::abs(result1->contactPointA.y() - result2->contactPointA.y());
