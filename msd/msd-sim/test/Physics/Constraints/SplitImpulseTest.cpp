...
TEST(SplitImpulse, VelocityRHS_NoBaumgarteBias)
{
  ...
  ContactConstraint constraint{
      ...
      restitution, 0.0};  // preImpactVel = 0 (resting)

  std::vector<TwoBodyConstraint*> constraints{&constraint};
  ...
  auto result =
      solver.solveWithContacts(constraints, states, inverseMasses,
                               inverseInertias, 2, dt);
...
TEST(SplitImpulse, VelocityRHS_RestitutionOnly)
{
  ...
  InertialState stateA = createDefaultState(
      Coordinate{0.0, 0.0, 0.0}, Coordinate{0.0, 0.0, 1.0});
  InertialState stateB = createDefaultState(Coordinate{0.0, 0.0, 1.0});

  ContactConstraint constraint{
      0, 1, normal, contactA, contactB, 0.01, comA, comB,
      0.5, 1.0};
...
TEST(SplitImpulse, PositionCorrection_ReducesPenetration)
{
  ...
  corrector.correctPositions(
      constraints, states, inverseMasses, inverseInertias,
      2, 2, 1.0 / 60.0, config);

  // Bodies should have moved apart along the normal
  double const separationChange =
      (stateB.position.z() - stateA.position.z()) -
...
TEST(SplitImpulse, PositionCorrection_DoesNotChangeVelocity)
{
  ...
  InertialState stateA = createDefaultState(
      Coordinate{0.0, 0.0, -0.5}, Coordinate{1.0, 2.0, 3.0});
...
TEST(SplitImpulse, Slop_NoCorrectionBelowThreshold)
{
  ...
  ContactConstraint constraint{
      0, 1, normal, contactA, contactB, 0.003, comA, comB,
      0.5, 0.0};
...
TEST(SplitImpulse, Slop_PartialCorrectionAboveThreshold)
{
  ...
  ContactConstraint constraintLarge{
      0, 1, normal, contactA, contactB, 0.025, comA, comB,
      0.5, 0.0};

  // Small penetration: 5mm above slop
  ContactConstraint constraintSmall{
      0, 1, normal, contactA, contactB, 0.010, comA, comB,
      0.5, 0.0};
  ...
  std::vector<TwoBodyConstraint*> constraints1{&constraintLarge};
  ...
  corrector.correctPositions(
      constraints1, states1, inverseMasses, inverseInertias,
      2, 2, 1.0 / 60.0, config);
  ...
  std::vector<TwoBodyConstraint*> constraints2{&constraintSmall};
  std::vector<InertialState*> states2{&stateA2, &stateB2};

  corrector.correctPositions(
      constraints2, states2, inverseMasses, inverseInertias,
      2, 2, 1.0 / 60.0, config);
...
TEST(SplitImpulse, EnergyConservation_RestingContact)
{
  ...
  for (int frame = 0; frame < 100; ++frame)
  {
    ...
    auto energy = EnergyTracker::computeSystemEnergy(
        world.getInertialAssets(), {});
...
TEST(SplitImpulse, SlopZero_FullCorrection)
{
  ...
  ContactConstraint constraint{
      0, 1, normal, contactA, contactB, 0.001, comA, comB,
      0.5, 0.0};  // 1mm penetration
...
TEST(SplitImpulse, MultiContact_IndependentCorrection)
{
  ...
  ContactConstraint c1{0, 1, normal,
      Coordinate{-0.5, -0.5, 0.0}, Coordinate{-0.5, -0.5, 0.0},
      0.015, comA, comB, 0.5, 0.0};  // 15mm (10mm above slop)
  ContactConstraint c2{0, 1, normal,
      Coordinate{0.5, -0.5, 0.0}, Coordinate{0.5, -0.5, 0.0},
      0.010, comA, comB, 0.5, 0.0};  // 10mm (5mm above slop)
  ContactConstraint c3{0, 1, normal,
      Coordinate{-0.5, 0.5, 0.0}, Coordinate{-0.5, 0.5, 0.0},
      0.003, comA, comB, 0.5, 0.0};  // 3mm (below slop)
  ContactConstraint c4{0, 1, normal,
      Coordinate{0.5, 0.5, 0.0}, Coordinate{0.5, 0.5, 0.0},
      0.020, comA, comB, 0.5, 0.0};  // 20mm (15mm above slop)

  std::vector<TwoBodyConstraint*> constraints{&c1, &c2, &c3, &c4};
  ...
  EXPECT_LT(stateA.position.z(), -0.5)
      << "Inertial body should move away from penetration (downward, since B is above)";
}
