...
TEST_F(DatabaseTest, OpenAndCreateDatabase)
{
  ...
  auto db = msd_db::Database(temp_db_path, test_logger,
                             msd_db::DBOpenCondition::OpenCreate);
...
TEST_F(DatabaseTest, CreateTableAndInsertData)
{
  ...
  sqlite3_stmt *stmt;
  int rc = sqlite3_prepare_v2(db.getRawDb(), "SELECT COUNT(*) FROM test_table",
                              -1, &stmt, nullptr);
...
TEST_F(DatabaseTest, TransactionCommit)
{
  ...
  ASSERT_TRUE(
      db.executeQuery("INSERT INTO test_table (value) VALUES ('row1')"));
  ASSERT_TRUE(
      db.executeQuery("INSERT INTO test_table (value) VALUES ('row2')"));
  ASSERT_TRUE(
      db.executeQuery("INSERT INTO test_table (value) VALUES ('row3')"));
...
TEST_F(DatabaseTest, TransactionRollback)
{
  ...
  ASSERT_TRUE(
      db.executeQuery("INSERT INTO test_table (value) VALUES ('permanent')"));
  ...
  ASSERT_TRUE(db.executeQuery(
      "INSERT INTO test_table (value) VALUES ('will be rolled back 1')"));
  ASSERT_TRUE(db.executeQuery(
      "INSERT INTO test_table (value) VALUES ('will be rolled back 2')"));
...
TEST_F(DatabaseTest, BuildDatabaseFactory)
{
  ...
  auto db_opt =
      msd_db::buildDatabase(temp_db_path, msd_db::DBOpenCondition::OpenCreate);
  ...
  ASSERT_TRUE(
      db_opt->executeQuery("CREATE TABLE test (id INTEGER PRIMARY KEY)"));
}
...
TEST_F(DatabaseTest, ForeignKeyConstraints)
{
  ...
  // Insert parent row
  ASSERT_TRUE(
      db.executeQuery("INSERT INTO parent (id, name) VALUES (1, 'parent1')"));

  // Insert valid child row should succeed
  ASSERT_TRUE(db.executeQuery(
      "INSERT INTO child (parent_id, value) VALUES (1, 'valid')"));

  // Insert invalid child row should fail
  ASSERT_FALSE(db.executeQuery(
      "INSERT INTO child (parent_id, value) VALUES (999, 'invalid')"));
}
