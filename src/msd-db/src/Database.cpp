...
Database::Database(std::string dbUrl, std::shared_ptr<spdlog::logger> logger,
      ...
      dbUrl_{dbUrl}
{
  ...
  int rc = sqlite3_open_v2(dbUrl.c_str(), &rawDb, static_cast<int>(openCond),
                           nullptr);
  ...
  if (this != &other)
    {
      ...
      if (logger_)
        {
          logger_->trace("Database moved via move assignment from: {}",
                         other.dbUrl_);
        }
...
{
  logger_->debug("Setting foreign keys to: {}",
