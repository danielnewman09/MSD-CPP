...
- **Changes required**:
  ...
  3. **Friction coefficient source**: `AssetInertial` and `AssetEnvironment` gain a `frictionCoefficient_` member (default 0.5), following the existing restitution pattern:
     - `AssetInertial`: New constructor overload `AssetInertial(hull, frame, mass, restitution, frictionCoefficient)`. Existing constructors default friction to 0.5 for backward compatibility.
     - `AssetEnvironment`: Same pattern — new constructor overload with friction parameter, default 0.5.
     - Accessor: `double getFrictionCoefficient() const` on both classes.
     - Combination: `ContactConstraintFactory` (or `CollisionPipeline::createConstraints()`) computes `mu_combined = sqrt(mu_A * mu_B)` (geometric mean), consistent with restitution combination.
     - Existing callers and tests: unaffected (default parameter preserves API compatibility).
...
```cpp
enum class RowType { Normal, Tangent };
...
struct FlattenedConstraints
{
  ...
  std::vector<RowType> rowTypes;     // Normal or Tangent per row (avoids vector<bool>)
  std::vector<double> restitutions;  // e per row (0 for friction rows)
  int numContacts{0};
};
```

The flattened structure feeds directly into the existing assembly pattern but with clear per-row metadata. The `rowTypes` enum determines whether the `-(1+e)` factor applies to the RHS (Normal rows only).
...
**Phase 3 (0052d): Integration + ECOS Removal** (decomposed into sub-commits, each leaves test suite green)
- **3a**: Add `frictionCoefficient_` to `AssetInertial` / `AssetEnvironment` with default 0.5 (no behavior change, all existing tests pass)
- **3b**: Implement constraint flattening + `solveWithFriction()` dispatch + `extractBodyForcesFlat()` in ConstraintSolver. Add `FrictionSpec.hpp`. Friction dispatch is dead code until CollisionPipeline creates FrictionConstraints. All existing tests pass.
- **3c**: Modify `CollisionPipeline` to create `FrictionConstraint` alongside `ContactConstraint`. Build interleaved `constraintPtrs_` and separate `normalConstraintPtrs_` for PositionCorrector. Friction constraints now flow through solver.
- **3d**: Modify `ContactCache` for 3 lambdas per contact + warm-start adaptation. Integration tests for cache round-trip.
- **3e**: Delete ECOS files, remove from CMakeLists and conanfile. Verify clean build with no ECOS references.
- **3f**: Integration tests verifying dispatch, flattening, negative lambda, energy monotonicity. Verify zero regressions on existing test suite (~693 tests).
...
- Phase 3 is decomposed into 6 sub-commits (3a-3f), each leaving the test suite green. This avoids a single monolithic changeset while ensuring no intermediate state breaks.
...
**Date**: 2026-02-10
**Status**: APPROVED (revisions applied by team-lead)
**Iteration**: 1 of 1
...
|----|-------|----------|-----------------|
| I1 | FrictionConeSolver Rule of Five is unnecessary | C++ Quality | Class has only scalar members with no custom resource management. Explicit Rule of Five declarations should be removed in favor of Rule of Zero. |
| I2 | `FrictionConeSolver::solve()` should be const but class design mixes configuration with solving | C++ Quality | The solver stores configuration as mutable member state (`tolerance_`, `max_iterations_`, etc.), but `solve()` is marked const. This prevents configuration changes during a solve, which is correct, but the setters are not thread-safe. Clarify thread-safety story or accept configuration as constructor parameters instead of setters. |
| I3 | `FlattenedConstraints` uses `std::vector<bool>` for `isNormalRow` | C++ Quality | `std::vector<bool>` is a well-known problematic specialization in C++. Use `std::vector<int>` or `std::vector<char>` instead, or better yet, use an enum for row type. |
| I4 | Missing FrictionConstraint creation details in CollisionPipeline | Feasibility | The design says CollisionPipeline creates FrictionConstraints "alongside ContactConstraints" but does not specify how friction coefficient is obtained from the colliding bodies. The user approved Option A (per-object member on AssetInertial/AssetEnvironment), but the design lacks the constructor signature change for AssetInertial/AssetEnvironment, and how existing callers and tests handle the new parameter. |
| I5 | Phase 3 atomicity risk is understated | Integration | Phase 3 (0052d) lists 11 steps including ECOS removal, friction creation, cache changes, and integration tests. Doing this "atomically" in one commit risks a very large, hard-to-review changeset. The design should decompose Phase 3 into sub-steps that maintain a green test suite at each commit. |
| I6 | `frictionConstraints_` member in PlantUML has wrong visibility | Architectural Fit | The PlantUML diagram shows `frictionConstraints_` as `public (+)` on CollisionPipeline but the design text describes it as a private member. This is inconsistent. |
| I7 | `ConeProjection` deleted default constructor is over-specified | C++ Quality | Making the constructor `= delete` to prevent instantiation is a valid pattern, but the class has no non-static members and all methods are static. A namespace with free functions would be more idiomatic C++ and simpler. However, this is consistent with the project pattern if other utility classes use static methods. Clarify whether the project has a precedent. |

### Revision Instructions for Architect

The following changes must be made before final review:

1. **Issue I1 (Rule of Zero for FrictionConeSolver)**: Remove the explicit copy/move constructors and assignment operators from `FrictionConeSolver`. The class contains only `double` and `int` members -- the compiler-generated defaults are correct. Per CLAUDE.md coding standards: "Prefer implementing NONE (Rule of Zero)." Only add an explicit comment noting Rule of Zero is intentional.

2. **Issue I3 (vector<bool> in FlattenedConstraints)**: Replace `std::vector<bool> isNormalRow` with a row-type indicator that avoids the `vector<bool>` specialization. Recommended: use an `enum class RowType { Normal, Tangent }` and `std::vector<RowType>` for clarity and type safety.

3. **Issue I4 (Friction coefficient on assets)**: Add explicit specification for:
   - The new constructor overload for `AssetInertial` that accepts `frictionCoefficient` (following the existing restitution pattern)
   - The equivalent for `AssetEnvironment`
   - Default value (0.5) behavior when not specified
   - The geometric mean combination formula location (which component computes `sqrt(mu_A * mu_B)`)

4. **Issue I5 (Phase 3 decomposition)**: Decompose Phase 3 into sub-commits that each leave the test suite green:
   - 3a: Add friction coefficient to AssetInertial/AssetEnvironment (no behavior change, existing tests pass)
   - 3b: Add FrictionConstraint creation to CollisionPipeline + FrictionSpec + constraint flattening in ConstraintSolver (solver dispatches to FrictionConeSolver when friction present)
   - 3c: Modify ContactCache for 3 lambdas per contact + warm-start adaptation
   - 3d: Add normalConstraintPtrs_ for PositionCorrector filtering
   - 3e: Delete ECOS files, update CMakeLists, update conanfile
   - 3f: Integration tests

5. **Issue I6 (PlantUML visibility)**: Fix the `+` (public) on `frictionConstraints_` to be `-` (private) in the PlantUML diagram.

### Items Passing Review (No Changes Needed)

The following aspects of the design pass review and should not be modified:

- **ConeProjection interface**: Well-designed stateless utility with clear three-case projection. The `project`, `gradient`, and `projectVector` signatures are clean and testable. Header-only placement is appropriate for small inline functions.

- **FrictionConeSolver algorithm**: The projected Newton approach with Armijo line search is well-established in the literature (MuJoCo, Drake). The algorithm description is clear and the convergence monitoring (projected gradient residual) is correct.

- **Constraint flattening strategy**: The rationale for pre-flattening multi-row constraints into 1-row-per-entry is sound. This preserves the existing `block<1,6>` assembly hot path without modification and localizes complexity to a single expansion step. The `FlattenedConstraints` struct is well-designed (pending the I3 fix).

- **ECOS removal plan**: Comprehensive and correct. All ECOS files, test files, build system references, and header includes are identified. No missed dependencies.

- **ContactCache 3-lambda adaptation**: The flat vector approach (3 values per contact at indices `[3i, 3i+1, 3i+2]`) is appropriate. The tangent basis sensitivity analysis is thoughtful -- relying on cone projection for feasibility restoration of stale tangential lambdas is correct.

- **PositionCorrector filtering**: Option A (explicit `normalConstraintPtrs_`) is the right choice. Clearer intent and avoids unnecessary dynamic_cast overhead.

- **Data flow sequence**: The pipeline phases (detect -> create constraints -> assemble -> warm-start -> solve -> cache update -> apply forces -> correct positions) are correctly ordered and match the existing CollisionPipeline architecture.

- **Test coverage plan**: The unit test cases for ConeProjection and FrictionConeSolver are thorough. The M8 examples provide concrete numerical validation. Integration tests cover the key correctness properties (dispatch, flattening, negative lambda, energy monotonicity).

- **FrictionSpec**: Lightweight, appropriate replacement for FrictionConeSpec. No ECOS dependency, plain data.

- **`extractBodyForces` fix**: Correctly identifies the `lambda(i) <= 0` skip as a bug for friction forces. The recommended approach (separate `extractBodyForcesFlat()` method) is clean -- it avoids modifying the existing no-friction code path.

- **Migration plan phases 1 and 2**: Correctly identified as independent and parallelizable. Phase 4 correctly depends on Phase 3.
