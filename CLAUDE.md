...
Automates generation of pybind11 bindings and Pydantic leaf models from msd-transfer C++ record headers. Eliminates manual boilerplate and ensures consistency across the Python bindings and REST API layers.

**Purpose**: Keep pybind11 bindings (`msd-pybind/src/record_bindings.cpp`) and Pydantic models (`replay/replay/generated_models.py`) synchronized with C++ record definitions in msd-transfer.

**Generator Script**: `scripts/generate_record_layers.py`

#### What Gets Generated

| Target File | Purpose | Pattern |
|-------------|---------|---------|
| `msd/msd-pybind/src/record_bindings.cpp` | pybind11 bindings for all 28+ transfer records | Mechanical patterns based on field type (primitive, nested, ForeignKey, RepeatedField) |
| `replay/replay/generated_models.py` | Pydantic leaf models for API consumption | C++ → Python type mapping, camelCase → snake_case transformation |

#### Usage via /sync-records Skill

The recommended workflow is through the `/sync-records` skill (`.claude/skills/sync-records/SKILL.md`):
...
This will:
1. Parse all `BOOST_DESCRIBE_STRUCT` macros from msd-transfer headers
2. Regenerate `record_bindings.cpp` and `generated_models.py`
3. Optionally run the 0061 cross-layer mapping indexer to update traceability
4. Report drift in hand-written composite models

#### Manual Generator Usage

For direct invocation or CI integration:
...
| `ForeignKey<T>` | `.def_property_readonly("name_id", [](const Record& r) { return r.name.id; })` | `name_id: int` |
| `RepeatedFieldTransferObject<T>` | `.def_property_readonly("name", [](const Record& r) { return r.name.data; })` | `name: list[T]` |

#### C++ → Pydantic Name Mapping

Multiple C++ records can map to the same Pydantic class when they are structurally equivalent:

```python
# From scripts/generate_record_layers.py NAME_MAPPING
"CoordinateRecord": "Vec3"     # position, velocity, acceleration all use Vec3
"VelocityRecord": "Vec3"
"Vector3DRecord": "Vec3"
...
The generator is integrated into the feature development workflow:

- **Developer workflow**: Use `/sync-records` skill after modifying transfer record headers
- **Docs-updater integration**: When a ticket touches `msd-transfer/src/*.hpp`, the docs-updater agent automatically runs the sync (Phase 6.5)
- **CI drift check**: Run `python scripts/generate_record_layers.py --check-only` in CI to fail if generated files are stale (recommended)

#### Design Decision Linkage

Generated Pydantic models include `Maps-to:` docstring annotations that link back to C++ records:
...
These annotations enable the 0061 cross-layer mapping indexer to deterministically link Python models to C++ records in the traceability database, eliminating heuristic matching.

**Introduced**: [Ticket: 0062_pybind_codegen_from_boost_describe](tickets/0062_pybind_codegen_from_boost_describe.md)
**Design**: [`docs/designs/0062_pybind_codegen_from_boost_describe/design.md`](docs/designs/0062_pybind_codegen_from_boost_describe/design.md)
